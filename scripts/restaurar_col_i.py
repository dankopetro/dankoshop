#!/usr/bin/env python3
"""
restaurar_col_i.py - Reconstruye la col K "Precio Ingresado" del maestro.

Contexto: sync_medusa_to_excel.py recreaba el Excel con openpyxl.Workbook()
y borraba la col K "Precio Ingresado". Este script la reconstruye
SIN borrar nada:

- Toma el maestro ACTUAL (con todos los SKUs, incluidos los nuevos + IDs Medusa).
- Para SKUs que existen en el recalc de LibreOffice (fuente con col K original):
    -> copia el valor de col K.
- Para SKUs nuevos (no presentes en el recalc):
    -> calcula K = round(F / 1.072) para reconstruir el precio ingresado.
- Reescribe la fórmula de la col F (=IF(K{r}="","",ROUND(K{r}*1.072,0)))
  en filas 2..10000.
...
Layout actual del Maestro:
  A=SKU, B=Artículo, C=Categoría, D=Descripción, E=PRECIO LISTA,
  F=Precio Mayorista, G=Envío Grande, H=Inventario, I=Canales de Venta,
  J=ID Medusa (hidden), K=Precio Ingresado
...
def load_recall_col_i():
    ...
    for r in range(2, ws.max_row + 1):
        ...
        if sku:
            out[str(sku).strip()] = ws.cell(r, 11).value
    return out
...
def main():
    ...
    # Escribir header de col K si no existe
    if ws.cell(1, 11).value is None:
        ws.cell(1, 11).value = "Precio Ingresado"
    ...
    for r in range(2, ws.max_row + 1):
        ...
        # 1) Si está en el recalc, copiar col K
        if sku in recalc:
            ws.cell(r, 11).value = recalc[sku]
            usados_recalc += 1
        # 2) Si es nuevo y F tiene valor, derivar K = round(F/1.072)
        elif isinstance(f_val, (int, float)) and f_val:
            ws.cell(r, 11).value = int(round(float(f_val) / FACTOR))
            nuevos.append(sku)
            calculados += 1

        # 3) Reescribir fórmula de F para mantenerla viva
        ws.cell(r, 6).value = f'=IF(K{r}="","",ROUND(K{r}*1.072,0))'

    # Asegurar fórmula F en TODAS las filas hasta 10000 (incluye futuras)
    for r in range(2, 10001):
        c = ws.cell(r, 6)
        if not c.value:
            c.value = f'=IF(K{r}="","",ROUND(K{r}*1.072,0))'

    wb.save(MAESTRO_PATH)
    print(f"SKUs procesados: {n_skus}")
    print(f"  Col K copiada desde recalc: {usados_recalc}")
    print(f"  Col K derivada (nuevos): {calculados}")
    print(f"  Nuevos con K derivada ({len(nuevos)}): {sorted(nuevos)[:60]}")
