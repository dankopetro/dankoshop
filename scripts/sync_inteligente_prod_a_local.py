#!/usr/bin/env python3
"""
sync_inteligente_prod_a_local.py - Sync inteligente Producción → Local
Solo crea, actualiza o salta productos. NUNCA borra.

Uso:
  python3 scripts/sync_inteligente_prod_a_local.py
"""
...
def main():
    print("=" * 60)
    print("  Sync Inteligente: Producción → Local")
    ...
    print("\nLeyendo inventario y canales de venta de Producción...")
    src_inv = fetch_source_inventory(PROD_URL, prod_token)
    print(f"  Inventario: {len(src_inv)} SKUs")
    src_sc = fetch_source_sales_channels(PROD_URL, prod_token)
    ...
    for i, (sku, src_product) in enumerate(prod_map.items(), 1):
        title = src_product.get("title", "?")[:40]
        src_norm = normalize_product(src_product)

        if sku in local_map:
            dest_product = local_map[sku]
            dest_norm = normalize_product(dest_product)

            if src_norm == dest_norm:
                print(f"  [{i}/{len(prod_map)}] {sku}: {title} → SALTAR")
                skipped += 1
            else:
                payload = build_payload(src_product)
                if update_product(LOCAL_URL, local_token, dest_product["id"], payload):
                    print(f"  [{i}/{len(prod_map)}] {sku}: {title} → ACTUALIZAR")
                    updated += 1
                    qty = src_inv.get(sku)
                    if qty is not None and dest_product.get("variants"):
                        sync_inventory(LOCAL_URL, local_token, dest_product["variants"][0]["id"], sku, qty)
                    ch = ", ".join(src_sc.get(src_product.get("id"), []))
                    if ch:
                        sync_sales_channels(LOCAL_URL, local_token, dest_product["id"], ch)
                else:
                    print(f"  [{i}/{len(prod_map)}] {sku}: {title} → ERROR")
                    errors += 1
        else:
            payload = build_payload(src_product)
            new_id = create_product(LOCAL_URL, local_token, payload)
            if new_id:
                print(f"  [{i}/{len(prod_map)}] {sku}: {title} → CREAR")
                created += 1
                qty = src_inv.get(sku)
                if qty:
                    r_prod = _api(LOCAL_URL, local_token, "GET", f"/admin/products/{new_id}")
                    if r_prod.status_code == 200:
                        variants = r_prod.json().get("product", {}).get("variants", [])
                        if variants:
                            sync_inventory(LOCAL_URL, local_token, variants[0]["id"], sku, qty)
                ch = ", ".join(src_sc.get(src_product.get("id"), []))
                if ch:
                    sync_sales_channels(LOCAL_URL, local_token, new_id, ch)
            else:
                print(f"  [{i}/{len(prod_map)}] {sku}: {title} → ERROR")
                errors += 1
    ...
    print(f"{'='*60}")
    print(f"\n  Para BORRAR productos, hacelo manualmente en Local")
